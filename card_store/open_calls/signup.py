...
def handle_request():
    ...
    dbQuery = sql.SQL("select {field} from {table} where {key} = %s").format(field = sql.Identifier('username'), table = sql.Identifier('accounts'),key = sql.Identifier('username'))
    ...
    if cur.fetchone() is None:
        ...
        dbQuery = sql.SQL("insert into {table} ({field1} , {field2}) values (%s , %s);").format(table = sql.Identifier('accounts'), field1 = sql.Identifier('username'),field2 = sql.Identifier('password'))
